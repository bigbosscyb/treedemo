namespace testdemo.useihierarchical;
...
public static class HierarchicalExtensions
{
    ...
    public static IHierarchical<T> AsHierarchical<T>(this T item, Func<T, IEnumerable<T>> childSelector,
        bool supportVariableDataSource = false)
    {
        return supportVariableDataSource
            ? new UpdateableHierarchical<T>(item, childSelector)
            : new Hierarchical<T>(item, childSelector);
    }
    ...
    private class Hierarchical<T> : IHierarchical<T>
    {
        ...
        public IEnumerable<IHierarchical<T>> Siblings =>
            Parent?.Children?.Where(node => node != this) ?? Array.Empty<IHierarchical<T>>();
        ...
        public int MaxDegreeOfTree =>
            Math.Max(Degree, Descendants.Any() ? Descendants.Select(node => node.Degree).Max() : 0);
    ...
    private class UpdateableHierarchical<T> : Hierarchical<T>
    {
        ...
        protected UpdateableHierarchical(T item, IHierarchical<T> parent, Func<T, IEnumerable<T>> childSelector) : base(
            item, parent, childSelector)
        {
        }
        ...
        private IEnumerable<IHierarchical<T>> UpdateChildren()
        {
            lock (_locker)
            {
                ...
                {
                    var searchItem =
                        _children?.SingleOrDefault(x => EqualityComparer<T>.Default.Equals(x.Current, item));
                    if (searchItem != null)
                    {
